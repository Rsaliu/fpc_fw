...
subcriber_event_task_t subcriber_event;
...
void app_main(void)
{
    ...
    level_sensor_config_t sensor_config1 = {.id = 1,.sensor_addr = 0x01, .protocol = protocol, 
            .medium_context = rs485_ttl, .send_recive = send_receive, .interpreter= interpret, tank_t* tank1 = NULL;
tank_t* tank2 = NULL;
...
void app_main(void)
{
    ...
    tank_monitor_event_callback_t event_callback = dummy_event_callback;

    tank_monitor_event_hook_t hook = {.context = NULL, .actuator_id = 2, .callback = event_callback};
    ...
    subcriber_event.monitor = monitors[1];
    subcriber_event.event_id = 3;
    subcriber_event.event_hook = hook;

    xTaskCreate(&tank_monitor_task,"tank_monitor_task",4096, &tank_monitor_task_config,1,NULL);
    xTaskCreate(&subscriber_event_task, "subscriber_event_task", 4096, &subcriber_event,2,NULL);};
