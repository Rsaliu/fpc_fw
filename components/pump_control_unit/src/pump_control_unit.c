...
static monitor_relay_group_t * get_group_from_tank_monitor_event_map(HashMap* map, int tank_monitor_id, int relay_id,int *event_id) {
    ...
    ESP_LOGI(TAG,"Searching for group with tank monitor ID: %d and relay ID: %d", tank_monitor_id, relay_id);
    ...
    while (entry != NULL) {
        ...
        if (group->monitor_id == tank_monitor_id && group->relay_id == relay_id) {
            *event_id = entry->key; // Set the event ID
            ESP_LOGI(TAG,"Found group for tank monitor ID: %d, relay ID: %d, event ID: %d", tank_monitor_id, relay_id, *event_id);
...
error_type_t pump_control_unit_add_tank_monitor(pump_control_unit_t *manager, tank_monitor_t *tank_monitor) {
    ...
    ESP_LOGI(TAG,"Adding tank monitor with ID: %d to the pump control unit", config.id);
    ESP_LOGI(TAG,"Tank monitor pointer value: %p", tank_monitor);
...
error_type_t pump_control_add_relay_to_tank_monitor(pump_control_unit_t *manager, int tank_monitor_id, int relay_id)
{
    ...
    ESP_LOGI(TAG,"Adding relay ID: %d to tank monitor ID: %d after confirming init state", relay_id, tank_monitor_id);
    if(!emhashmap_contains(&manager->tank_monitors_map, tank_monitor_id)) {
        return SYSTEM_INVALID_PARAMETER; // Tank monitor not found
    }
    ESP_LOGI(TAG,"Tank monitor ID: %d found, proceeding to add relay ID: %d", tank_monitor_id, relay_id);
    ...
    ESP_LOGI(TAG,"Subscribed to tank monitor ID: %d with event ID: %d", tank_monitor_id, event_id);
    ...
    ESP_LOGI(TAG,"Added relay ID: %d to tank monitor ID: %d with event ID: %d", relay_id, tank_monitor_id, event_id);
...
error_type_t pump_control_unit_remove_relay_from_tank_monitor(pump_control_unit_t *manager, int tank_monitor_id, int relay_id){
    ...
    ESP_LOGI(TAG,"Removing relay ID: %d from tank monitor ID: %d after confirming init state", relay_id, tank_monitor_id);
    if(!emhashmap_contains(&manager->tank_monitors_map, tank_monitor_id)) {
        return SYSTEM_INVALID_PARAMETER; // Tank monitor not found
    }
    ESP_LOGI(TAG,"Tank monitor ID: %d found, proceeding to remove relay ID: %d", tank_monitor_id, relay_id);
    ...
    if (grp == NULL || event_id < 0) {
        ESP_LOGE(TAG,"Failed to find group for tank monitor ID: %d, relay ID: %d", tank_monitor_id, relay_id);
...
//     if (grp == NULL || event_id < 0) {
//         ESP_LOGE(TAG,"Failed to find group for pump monitor ID: %d, relay ID: %d", pump_monitor_id, relay_id);
...
void callback_handler(void* context,int actuator_id, event_type_t state,int monitor_id){
    ...
    ESP_LOGI(TAG,"Tank state changed to: %d for monitor ID: %d", state, monitor_id);
    relay_t *relay = NULL;
    ...
    if (err != SYSTEM_OK || relay == NULL) {
        ESP_LOGE(TAG,"Failed to get relay for actuator ID: %d", actuator_id);
        return; // Handle error in getting relay
    }
    // Here you can control the relay based on the state
    if (state == EVENT_TANK_FULL_STATE) {
        ESP_LOGI(TAG,"Turning on relay for actuator ID: %d", actuator_id);
        // Code to turn on the relay
    } else if (state == EVENT_TANK_LOW_STATE) {
        ESP_LOGI(TAG,"Turning off relay for actuator ID: %d", actuator_id);
        // Code to turn off the relay
    } else {
        ESP_LOGI(TAG,"No action for state: %d", state);
    }
