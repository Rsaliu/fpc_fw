...
error_type_t pump_control_unit_add_subscriber_to_pump_monitor(pump_control_unit_t *manager, int pump_monitor_id, const pump_monitor_subscriber_t* subscriber,int* event_id){
    if (manager == NULL || subscriber == NULL || event_id == NULL) {
        return SYSTEM_NULL_PARAMETER; // Handle null manager, subscriber or event_id pointer
    }
    if(pump_monitor_id < 0) {
        return SYSTEM_INVALID_PARAMETER; // Invalid pump monitor ID
    }
    if(!manager->is_initialized) {
        return SYSTEM_INVALID_STATE; // Manager is not initialized
    }
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Adding subscriber to pump monitor ID: %d after confirming init state", pump_monitor_id);
    if(!emhashmap_contains(&manager->pump_monitors_map, pump_monitor_id)) {
        return SYSTEM_INVALID_PARAMETER; // Pump monitor not found
    }
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Pump monitor ID: %d found, proceeding to add subscriber", pump_monitor_id);
    MapEntry* map_entry = emhashmap_get(&manager->pump_monitors_map, pump_monitor_id);
    if (map_entry == NULL) {
        return SYSTEM_INVALID_PARAMETER; // Pump monitor not found
    }
    pump_monitor_t *pump_monitor = (pump_monitor_t *)map_entry->value;
    error_type_t err = pump_monitor_subscribe_event(pump_monitor, subscriber, event_id);
    if (err != SYSTEM_OK) {
        return err; // Handle error in subscribing to the event
    }
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Subscribed to pump monitor ID: %d with event ID: %d", pump_monitor_id, *event_id);
    
    return SYSTEM_OK;
}

error_type_t pump_control_unit_remove_subscriber_from_pump_monitor(pump_control_unit_t *manager, int pump_monitor_id, int event_id){
    if (manager == NULL) {
        return SYSTEM_NULL_PARAMETER; // Handle null manager
    }
    if(pump_monitor_id < 0 || event_id < 0) {
        return SYSTEM_INVALID_PARAMETER; // Invalid pump monitor ID or event ID
    }
    if(!manager->is_initialized) {
        return SYSTEM_INVALID_STATE; // Manager is not initialized
    }
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Removing subscriber with event ID: %d from pump monitor ID: %d after confirming init state", event_id, pump_monitor_id);
    if(!emhashmap_contains(&manager->pump_monitors_map, pump_monitor_id)) {
        return SYSTEM_INVALID_PARAMETER; // Pump monitor not found
    }
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Pump monitor ID: %d found, proceeding to remove subscriber with event ID: %d", pump_monitor_id, event_id);
    MapEntry* map_entry = emhashmap_get(&manager->pump_monitors_map, pump_monitor_id);
    if (map_entry == NULL) {
        return SYSTEM_INVALID_PARAMETER; // Pump monitor not found
    }
    pump_monitor_t *pump_monitor = (pump_monitor_t *)map_entry->value;
    error_type_t err = pump_monitor_unsubscribe_event(pump_monitor, event_id);
    if (err != SYSTEM_OK) {
        return err; // Handle error in unsubscribing from the event
    }
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Unsubscribed from pump monitor ID: %d with event ID: %d", pump_monitor_id, event_id);
    return SYSTEM_OK;
}

error_type_t pump_control_unit_loop_pump_monitors(pump_control_unit_t *manager){
    if (manager == NULL) {
        return SYSTEM_NULL_PARAMETER; // Handle null manager
    }
    if(!manager->is_initialized) {
    ...
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Looping through pump monitors to check for events");
    MapIterator it = emhashmap_iterator(&manager->pump_monitors_map);
    MapEntry* entry = emhashmap_iterator_next(&it);
    while (entry != NULL) {
        pump_monitor_t *pump_monitor = (pump_monitor_t *)entry->value;
        error_type_t err = pump_monitor_check_current(pump_monitor);
        if (err != SYSTEM_OK) {
            ESP_LOGE(PUMP_CONTROL_UNIT_TAG,"Error checking current for pump monitor ID: %d, error code: %d", entry->key, err);
        } else {
            ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Checked current for pump monitor ID: %d successfully", entry->key);
        }
        entry = emhashmap_iterator_next(&it); // Move to the next entry
    }
    return SYSTEM_OK;
...
error_type_t pump_control_unit_loop_level_monitors(pump_control_unit_t *manager){
    if (manager == NULL) {
        return SYSTEM_NULL_PARAMETER; // Handle null manager
    }
    if(!manager->is_initialized) {
    ...
    ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Looping through tank monitors to check for events");
    MapIterator it = emhashmap_iterator(&manager->tank_monitors_map);
    MapEntry* entry = emhashmap_iterator_next(&it);
    while (entry != NULL) {
        tank_monitor_t *tank_monitor = (tank_monitor_t *)entry->value;
        error_type_t err = tank_monitor_check_level(tank_monitor);
        if (err != SYSTEM_OK) {
            ESP_LOGE(PUMP_CONTROL_UNIT_TAG,"Error checking level for tank monitor ID: %d, error code: %d", entry->key, err);
        } else {
            ESP_LOGI(PUMP_CONTROL_UNIT_TAG,"Checked level for tank monitor ID: %d successfully", entry->key);
        }
        entry = emhashmap_iterator_next(&it); // Move to the next entry
    }
    return SYSTEM_OK;
