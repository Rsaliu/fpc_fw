...
error_type_t dummy_read_current_value(void* context, float* current_value) {
...
void pumpMonitorSetUp(void)
{
    ...
    current_sensor_config_t current_sensor_config = {
        ...
        .read_current = dummy_read_current_value,       // Assuming a function pointer to read current is set later
    };
